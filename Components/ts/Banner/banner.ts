...
export class Banner extends AbstractComponent {
    create(_banner: HTMLElement, _title: string, _desc: string, _anchor: IAnchor): HTMLElement | null {
        try {
            ...
            if (title) _banner.appendChild(title);
            if (desc) _banner.appendChild(desc);
            if (anchor) {
